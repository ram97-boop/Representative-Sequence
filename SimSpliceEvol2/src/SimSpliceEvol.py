...
def make_protein_evol(my_iteration, tree, exon_intron_of_genes, resulting_gene_exon, tc5, tc3, tcsk, tcme, tc6, k3, intron_dict, path, iteration, genes_seq):
	...
	for node in tree.traverse("preorder"):
		...
		if node.is_root():
			...
			#for e in exon
			cds_name = "cds_" + str(my_iteration)
			cds_list[node.name[3]][cds_name] = [cds_0, {}, {}]
			cds_list_gene[cds_name] = [cds_0, {}, {}]
...
def write_datas(leaves, cds_list, exon_intron_of_genes, resulting_gene_exon,intron_dict, path, iteration, genes_seq):
	cds_file = open(path + "cds/" + iteration + "cds.fasta", "a")
	exon_positions_file = open(path + "positions/" + iteration + "exon_positions.fasta", "a")
	cds_to_gene_file = open(path + "cds_gene/" + iteration + "cds_gene.txt", "a")	
	alignement_pair = open(path + "pairwise_alignment/" + iteration + "alignement.fasta", "a")
	cluster = open(path + "cluster/" + iteration + "cluster.fasta", "a")
	
	alignement_multiple = open(path + "multiple_alignment/" + iteration + "multiple.fasta", "a")
...
def main(gain, dup, lost, iteration, arg):
	...
	for i in range(5):
	    make_protein_evol(i, tree, exon_intron_of_genes, resulting_gene_exon, tc5, tc3, tcsk, tcme, tc6, k3, intron_dict, src, src2, genes_seq)
