...
grep -n peptide "$1" > seq_info.txt &&
	if [ $rep_seq == $(cat seq_info.txt | wc -l) ] # If the last sequence is the representative.
		then
		...
		# sequence and the sequence under it in tmp.txt
		...
		# Retrieve the id line and the sequence of the representative sequence.
